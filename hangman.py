...
def replacing(counter, empty_word, letters_word, user_try):
    new_list = empty_word.split(" ")
    for repl in range(counter):
        list_index = letters_word.index(user_try)
        del new_list[list_index]
        new_list.insert(list_index, user_try)
    empty_word = " ".join(new_list)
    return empty_word

def play(word):
    empty_word = "_ " * (len(word) - 1) + "_"
    ...
    while True:
        ...
        if tries == 0:
            print("Вы проиграли!\n")
            print("Это было слово:", word, end="\n")
            break
        user_try = input(("Назовите букву или слово целиком: "))
        user_try = user_try.upper()
        if user_try == word:
            print("Вы угадали слово:\n", word)
            break
        elif user_try in letters_word and user_try.isalpha():
            print("Вы угадали букву\n")
            counter = letters_word.count(user_try)
            empty_word = replacing(counter, empty_word, letters_word, user_try)
        elif user_try not in letters_word and user_try.isalpha():
...
while True:
    ...
    question = input("Да - чтобы сыграть , Нет - чтобы закончить: ")
    if question == "да".lower():
        word = get_word()
        play(word)
    elif question == "нет".lower():
        break
    else:
